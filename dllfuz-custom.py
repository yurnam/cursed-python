...
WORKER_TIMEOUT_SEC = 60  # timeout to check and respawn workers
...
# Optional, but helps DLL dependency resolution: prepend target DLL's dir to PATH
...
def get_target_dll_path():
    ...
    if selected_path and validate_dll_path(selected_path):
        print(f"[+] Using selected DLL path: {selected_path}")
...
def orchestrate():
    ...
    target_dll_path = get_target_dll_path()
