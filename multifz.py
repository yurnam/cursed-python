#!/usr/bin/env python3
# DLL Folder Fuzzer - selects a folder and fuzzes DLLs with non-destructive functions
#
# Features:
# - Selects a folder containing DLLs via dialog
# - Scans the folder for DLLs
# - Fuzzes each DLL sequentially for ~20 seconds using non-destructive functions
...
# ==== HARD-CODED CONFIG ========================================
WORKERS = 1000  # parallel processes for function execution
TOTAL_DURATION_SEC = 1200  # total runtime
FUZZ_PER_DLL_SEC = 4  # fuzz each DLL for ~20 seconds
MAX_ARGS_PER_CALL = 9  # 0..N args
MAX_RANDOM_BUF_BYTES = 1087  # 1MB max buffer size for pointer args
RNG_SEED = None  # set to an int for reproducible chaos, or None
WORKER_TIMEOUT_SEC = 1  # timeout to check and respawn workers

# --- TIMING CONTROLS ---
SHUFFLE_INTERVAL_SEC = 2  # shuffle function array every 12 seconds
RANDOMIZE_INTERVAL_SEC = 3  # re-randomize parameter data every 13 seconds
EXECUTION_BATCH_SIZE = 1000  # preferred batch size (but not required)

# Optional, but helps DLL dependency resolution: prepend DLL's dir to PATH
PREPEND_DLL_DIR_TO_PATH = True

# Blacklist of potentially destructive function prefixes
...
def select_folder():
    ...
    try:
        ...
        folder_path = filedialog.askdirectory(
            title="Select folder containing DLL files"
        )
...
def scan_dlls_in_folder(folder_path):
    dlls = []
...
def enumerate_target_dll_functions(target_dll_path):
    ...
    # Filter non-destructive functions
    safe_functions = [func for func in function_names if
                      not any(func.startswith(prefix) for prefix in DESTRUCTIVE_PREFIXES)]
...
def worker_process(target_dll_path, dll_function_array, files_list):
    ...
    while True:
        ...
        for func_name, param_set in zip(functions_to_execute, param_sets_to_use):
            print(f"[*] Executing {func_name} with params: {param_set}")
            try:
...
def orchestrate():
    ...
    dlls = scan_dlls_in_folder(folder_path)
    if not dlls:
        sys.exit(1)

    files = scan_random_files(folder_path)
    if not files:
    ...
    while time.time() - start_time < TOTAL_DURATION_SEC:
        target_dll_path = dlls[dll_index % len(dlls)]
        dll_index += 1
        ...
        while time.time() - fuzz_start < FUZZ_PER_DLL_SEC:
            ...
            for p in processes:
                if p.is_alive():
                    ...
                    np = mp.Process(target=worker_process, args=(target_dll_path, dll_function_array, files), daemon=True)
                    np.start()
